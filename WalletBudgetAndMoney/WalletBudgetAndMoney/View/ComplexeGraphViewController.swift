...
class ComplexeGraphViewController: UIViewController {
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            reportDetail.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            reportDetail.trailingAnchor.constraint(equalTo: view.trailingAnchor),
            reportDetail.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -50)
            ])
    }
    ...
    private func configureGraphView() {
        ...
        let dataFrame = CGRect(x: x, y: 0, width: 7, height: stepY)
        let dataArea  = CGRect(x: 0, y: 0, width: model.reports.count + 1, height: Int(stepY))
        
        // Add decoration
        
        let grid = Grid(stepX: 1.0, stepY: stepY)
        ...
        let verticalAxis = VerticalAxis(step: stepY, position: .left)
