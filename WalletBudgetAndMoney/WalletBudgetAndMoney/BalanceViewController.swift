...
class BalanceViewController: UIViewController, UICollectionViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let balanceTrendCellRegistration = UICollectionView.CellRegistration<BalanceTrendCollectionViewCell, CellItem> {
            ...
            cell.buttonShowMoreBalanceTrend.isHidden = true
        }
        
        let balanceCellRegistration = UICollectionView.CellRegistration<CashFlowCollectionViewCell, CellItem>{
            cell, indexPath, itemIdentifier in
            cell.buttonShowMoreCashFlow.isHidden = true
            cell.labelCashFlow.attributedText = UIView.stringToNSAttributedString(string: "Счета", size: 26, weight: .bold, color: .black)
            cell.labelDateCashFlow.attributedText = UIView.stringToNSAttributedString(string: "Сегодня", size: 17, weight: .regular, color: .gray)
        }
        ...
        dataSource = UICollectionViewDiffableDataSource(collectionView: collectionView) { (collectionView: UICollectionView, indexPath: IndexPath, itemIdentifier: CellItem) -> UICollectionViewCell? in
            if indexPath.row == 0 {
                ...
                let cell = collectionView.dequeueConfiguredReusableCell(using: balanceCellRegistration, for: indexPath, item: itemIdentifier)
                return cell
            }
